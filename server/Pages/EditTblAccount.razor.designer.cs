...
{
    public partial class EditTblAccountComponent : ComponentBase
    {
        ...
        [Parameter]
        public dynamic gl_account_no { get; set; }
        ...
        {
            var accountingDbGetTblAccountByglAccountNoResult = await AccountingDb.GetTblAccountByglAccountNo(gl_account_no);
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Accounting.Models.AccountingDb.TblAccount args)
        {
            try
            {
                var accountingDbUpdateTblAccountResult = await AccountingDb.UpdateTblAccount(gl_account_no, tblaccount);
                DialogService.Close(tblaccount);
            }
            catch (System.Exception accountingDbUpdateTblAccountException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update TblAccount" });
            }
        }
