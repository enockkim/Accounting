...
namespace Accounting.Pages
{
    public partial class AddCustomerComponent : ComponentBase
    {
        ...
        {
            customer = new Accounting.Models.AccountingDb.Customer(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Accounting.Models.AccountingDb.Customer args)
        {
            try
            {
                var accountingDbCreateCustomerResult = await AccountingDb.CreateCustomer(customer);
                DialogService.Close(customer);
            }
            catch (System.Exception accountingDbCreateCustomerException)
            {
